    ...
    if (isset($message['text'])) {
        ...
        switch (true) {
            case preg_match('/^(\/set) @([\w]+) (\d.+)/ui ', $text, $matches):
                if ($from_id == "32512143") if (SetCarma($chat_id, GetUserID($matches[2]), $matches[3])) apiRequest("sendMessage", array('chat_id' => $from_id, "text" => "� " . $matches[2] . " (" . GetUserID($matches[2]) . ") � ���� " . $chat_id . " ����� " . $matches[3]));
                break;
            ...
            case preg_match('/^\/Stats/ui', $text, $matches):
                ...
                $out .= "<a href='" . PATH_TO_SITE . "?group_id=" . $chat_id . "'>���������</a>";
                apiRequest("sendChatAction", array('chat_id' => $chat_id, "action" => "typing"));
                apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => $out, "parse_mode" => "HTML"));

                break;
            case preg_match('/��\?? ?��/ui', $text, $matches):
                apiRequest("sendChatAction", array('chat_id' => $chat_id, "action" => "typing"));
                apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => "��. " . Punish($from_id, $chat_id), "parse_mode" => "HTML"));
                break;
            case preg_match('/([�|x|h|?][�|e]\W?){2}/ui', $text, $matches):
                apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => "��� ���� � ����. " . Punish($from_id, $chat_id), "parse_mode" => "HTML"));
                break;
            ...
            case preg_match('/^(\+|\-|?|?) ?([\s\S]+)?/ui', $text, $matches):
                ...
                if (isset($message['reply_to_message'])) {
                    ...
                    if (preg_match('/@([\w]+)/ui', $matches[2], $user)) {
                        $to = GetUserID($user[1]);
                        $to ? $output = HandleKarma($level, $from_id, $to, $chat_id) : $output = "� ��� �� ����, ������� ����� �� ����";
                        apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => $output, "parse_mode" => "HTML"));
                    }
                }
                break;
