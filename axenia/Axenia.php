    ...
    public function sendStore($chat_id, $from = NULL, $message = NULL, $text = NULL, $callback = NULL, $callback_id = NULL)
    {
        ...
        $button_list[] = [
            [
                ...
                'text' => Lang::message('store.button.buy_jokes'),
                'callback_data' => 'buy_jokes' . '|' . $from['id'] . '|' . '10'
            ]
        ];
        ...
        if ($message == NULL && $text == NULL) {
            ...
            if ($newKarma >= 0) {
                if($command[0]=="buy_jokes"){
                    Request::sendHtmlMessage($chat_id,$text, ['reply_to_message_id' => $message_id]);
                }else{
            ...
            if ($data_array[1] == $from['id']) {
                switch ($data_array[0]) {
                    ...
                    case 'buy_cats':
                        ...
                        break;
                    case 'buy_jokes':
                        $joke = json_decode(file_get_contents("http://www.umori.li/api/random?num=1"), true);
                        $rez = $joke["elementPureHtml"];
                        break;
                    default:
                        $rez = $data;
                }
