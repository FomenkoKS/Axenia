        ...
        if ($this->needToHandle($message)) {
            ...
            if (isset($message['text']) || isset($message['sticker'])) {
                ...
                switch (true) {
                    ...
                    case Util::startsWith($text, ("/debug")):
                        break;
                }
            } elseif (isset($message['new_chat_member'])) {
                $newMember = $message['new_chat_member'];
                if (array_key_exists('username', $newMember)) {
    ...
    public function processCallback($callback)
    {
        ...
        if (in_array($data, array_keys(Lang::availableLangs()))) {
            ...
            if ($data_array[1] == $from['id']) {
                switch ($data_array[0]) {
                    ...
                    case 'buy_butts':
                        ...
                        break;
                    case 'buy_bashorg':
                        $content = iconv("CP1251", "UTF-8", file_get_contents("http://bashorg.org/casual"));
                        $content = substr($content, strpos($content, "<div class=\"q\">"), -1);
                        $content = substr($content, strpos($content, "<div>"), -1);
                        $content = substr($content, 0, strpos($content, "<hr class=\"iq\">"));
                        $content = str_replace("<br />", "\r\n", $content);
                        $content = html_entity_decode($content);
                        $rez = strip_tags($content);
                        break;
                    case 'buy_jokes':
            ...
            if ($isAdminInChat) {
                ...
                Request::answerCallbackQuery($callback['id'], Lang::message('settings.titleGroup'));
            }
